...
import { spritePng, MOTIONS } from '../pets/registry.js';
import { SPRITE_META } from '../pets/spriteMeta.generated.js';
...
function metaFor(petId, stage, motion) { return SPRITE_META[`${petId}:${stage}:${motion}`]; }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        stateRef.current = { motion: 'idle', startMs: performance.now(), img: idleImg, meta: metaFor(pet.petId, stage, 'idle') };
        setReady(true);
        // click/click2/attack 백그라운드 프리로드(실패 무시 → 없으면 그 모션만 스킵)
        for (const m of MOTIONS) {
          if (m === 'idle') continue;
          loadImage(spritePng(pet, stage, m)).then((im) => { imagesRef.current[m] = im; }).catch(() => {});
        }
      } catch (e) {
        if (!alive) return;
