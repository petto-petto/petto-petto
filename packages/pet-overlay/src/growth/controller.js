...
import {
  SESSION_IDLE_TIMEOUT_SEC, TOKENS_PER_XP,
  SESSION_STEP_MIN, SESSION_STEP_BONUS, SESSION_BONUS_MAX,
} from './constants.js';
...
export class GrowthController {
  ...
  applyNow({ tokens, timestamp, eventId } = {}) {
    ...
    if (eventId != null) {
      if (this.seen.has(eventId)) return { tokens: 0, gained: 0, leveledUp: false, events: [], pet: this.pet, toNext: this._toNext() };
    ...
    if (deltaBase <= 0) return { tokens, gained: 0, leveledUp: false, events: [], pet: this.pet, toNext };

    const gain = Math.floor(deltaBase * this._sessionCoef(now));
    const res = applyXp(this.pet, gain);
    this.pet = res.pet;
    return { tokens, gained: gain, leveledUp: res.leveledUp, events: res.events, pet: res.pet, toNext };
  }
