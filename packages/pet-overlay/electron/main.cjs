...
function statePath() { return path.join(app.getPath('userData'), STATE_FILE); }
function loadWindowState() { try { return JSON.parse(fs.readFileSync(statePath(), 'utf8')); } catch { return null; } }
function saveWindowState(bounds) {
  try { fs.writeFileSync(statePath(), JSON.stringify({ x: bounds.x, y: bounds.y })); } catch { /* ignore */ }
}
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/ingest/claude-code') {
      ...
      req.on('end', () => {
        ...
        try {
          fs.appendFileSync(path.join(__dirname, '..', 'hook-ingest.log'),
...
app.whenReady().then(() => { createWindow(); startHookServer(); });
