/**
 * # @pet/stubs — 다른 도메인의 대역
 *
 * `collection`, `gacha`, `battle`, `overlay-growth`는 팀의 다른 사람이 만든다. 그 패키지가
 * 아직 없으므로 `@pet/meta`의 포트를 인메모리로 구현해 둔다.
 *
 * ## 이것이 별도 패키지인 이유
 *
 * `@pet/meta` 안에 넣으면 "내 도메인이 남의 도메인 구현을 들고 있는" 모양이 된다.
 * 앱 안에 넣으면 테스트에서 쓸 수 없다. 별도 패키지로 두면 **앱은 일반 의존으로, 테스트는
 * dev 의존으로** 같은 대역을 쓰고, 실제 도메인이 완성되면 앱의 의존 한 줄만 바꾸면 된다.
 *
 * `@pet/meta`의 포트를 구현하므로 `@pet/meta`를 의존한다. 즉 **어댑터 자리**다. 실제
 * `@pet/currency`는 meta의 인터페이스를 모른 채 자기 API를 갖고, 앱이 그 사이를 잇는다.
 *
 * ## 실패 주입
 *
